// ===================================
// GLANZ Wellness Hub - Translations
// ===================================

const translations = {
    en: {
        // Navigation
        nav_home: "Home",
        nav_about: "About",
        nav_products: "Products",
        nav_benefits: "Benefits",
        nav_bundles: "Bundles",
        nav_contact: "Contact",

        // Hero Section
        hero_badge: "Plant Based Hair Care",
        hero_title_1: "Unlock Your",
        hero_title_2: "Natural Radiance",
        hero_subtitle: "Experience the luxury of professional hair care with GLANZ Wellness Hub. Our scientifically formulated products nourish, repair, and transform your hair from within.",
        hero_btn_primary: "Explore the Collection",
        hero_btn_secondary: "Discover our Philosophy",
        hero_scroll: "Scroll to discover",

        // About Section
        about_label: "About GLANZ",
        about_tagline: "Wellness and Beauty Rooted in Every Strand",
        about_text_1: "GLANZ Wellness Hub was born from a simple yet powerful belief: that hair care should honor both beauty and wellness. What began as a passion for healthier hair evolved into a commitment to creating products that care deeply for the scalp, strengthening the hair, and elevate everyday routines into meaningful rituals. We exist at the intersection of wellness, science, and nature—creating plant-based, thoughtfully formulated hair care designed to support scalp health, encourage healthier hair growth, and restore balance from root to tip.",
        about_text_2: "As the founder, my vision has always been intentional growth—developing hair care that is organic, gentle, effective, and rooted in thoughtful formulation. Every product reflects a dedication to quality, balance, and long-term hair health, guided by the understanding that true results come from consistency and care. Because true hair health is a practice, not a promise.",
        about_text_3: "GLANZ is more than a brand; it is a philosophy of refinement, integrity, and purpose. We are committed to quality, transparency and sustainable growth. From ingredient selection to product development, every decision is guided by our dedication to excellence, wellness, and longevity. As we continue to expand, our focus remains clear: to build a trusted hair wellness brand that delivers excellence without compromise and creates lasting value for our community.",
        about_thank_you: "Thank you for being part of this journey.",
        about_founder: "— Founder, GLANZ Wellness Hub",

        // Mission & Vision
        mission_title: "Our Mission",
        mission_text: "To create refined, plant-based hair care that supports scalp health, encourages healthier hair growth, and transforms everyday routines into elevated wellness rituals through thoughtful formulation and intentional care.",
        vision_title: "Our Vision",
        vision_text: "To become a globally recognized hair wellness brand known for blending nature-inspired ingredients with modern formulation principles—setting a new standard for luxury, integrity, and long-term hair health.",

        // Values
        values_title: "Our Values",
        value_1_title: "Wellness First",
        value_1_text: "We prioritize scalp health, balance, and long-term results in every formulation and decision.",
        value_2_title: "Integrity & Trust",
        value_2_text: "We believe trust is built through honesty, quality, and consistency—from ingredients to brand experience.",
        value_3_title: "Sustainability & Excellence",
        value_3_text: "We are committed to responsible practices and building a brand designed for longevity and global relevance.",

        // Products Section
        products_label: "Our Collection",
        products_title_1: "ORGANIC",
        products_title_2: "HAIR CARE PRODUCTS",
        products_subtitle: "Discover our carefully curated range of professional-grade hair care solutions",

        product_1_tag: "Best Seller",
        product_1_title: "Anti-Hair Loss Nourishing Shampoo",
        product_1_desc: "Anti-frizz formula designed to promote hair growth and repair damaged strands. Enriched with nourishing ingredients for stronger, healthier hair.",
        product_1_size: "500ml",
        product_1_link: "Learn More →",

        product_2_tag: "Deep Treatment",
        product_2_title: "Keratin Repair Hair Mask",
        product_2_desc: "Intensive rejuvenation treatment for ultimate smoothness. Restores damaged hair with deep keratin infusion for silky, manageable locks.",
        product_2_size: "500ml",
        product_2_link: "Learn More →",

        product_3_tag: "Organic Oil",
        product_3_title: "Hair Therapy Oil",
        product_3_desc: "Luxurious therapy oil that restores and promotes hair growth. Lightweight formula absorbs quickly, leaving hair nourished without greasiness.",
        product_3_size: "250ml",
        product_3_link: "Learn More →",

        product_4_tag: "Accessory",
        product_4_title: "Silicone Scalp Brush",
        product_4_desc: "Ergonomic scalp massager designed to stimulate blood circulation and enhance product absorption. Available in elegant neutral tones.",
        product_4_size: "3 Colors",
        product_4_link: "Learn More →",

        // Benefits Section
        benefits_label: "Why Choose GLANZ",
        benefits_title_1: "Experience the",
        benefits_title_2: "Difference",

        benefit_1_title: "Plant Based Quality",
        benefit_1_text: "Every product is formulated with the finest ingredients, ensuring exceptional quality and visible results.",

        benefit_2_title: "Gentle Formulas",
        benefit_2_text: "Carefully balanced formulas that are gentle on your hair and scalp while delivering powerful results.",

        benefit_3_title: "Scientifically Proven",
        benefit_3_text: "Backed by research and developed with advanced hair science to target specific concerns effectively.",

        benefit_4_title: "Long-Lasting Results",
        benefit_4_text: "Experience transformative results that last, with consistent use revealing healthier, more vibrant hair.",

        // Gallery Section
        gallery_label: "Gallery",
        gallery_title_1: "See Our Products in",
        gallery_title_2: "Action",

        // Bundles Section
        bundles_label: "Complete Solutions",
        bundles_title_1: "Curated Hair Care",
        bundles_title_2: "Bundles",
        bundles_subtitle: "Get the complete GLANZ experience with our specially curated bundles",

        bundle_1_tag: "Starter Set",
        bundle_1_badge: "",
        bundle_1_title: "The Essential Duo",
        bundle_1_desc: "Perfect for beginners. Includes our Anti-Hair Loss Nourishing Shampoo and Keratin Repair Hair Mask for a complete cleansing and treatment routine.",
        bundle_1_item_1: "Anti-Hair Loss Nourishing Shampoo (500ml)",
        bundle_1_item_2: "Keratin Repair Hair Mask (500ml)",
        bundle_1_btn: "Shop Bundle",

        bundle_2_tag: "Complete Care",
        bundle_2_badge: "Most Popular",
        bundle_2_title: "The Ultimate Trio",
        bundle_2_desc: "Our most popular bundle. Complete your hair care ritual with all three core products plus two bonus scalp massagers for enhanced results.",
        bundle_2_item_1: "Anti-Hair Loss Nourishing Shampoo (500ml)",
        bundle_2_item_2: "Keratin Repair Hair Mask (500ml)",
        bundle_2_item_3: "Hair Therapy Oil (250ml)",
        bundle_2_item_4: "2x Silicone Scalp Massagers",
        bundle_2_btn: "Shop Bundle",

        bundle_3_tag: "Treatment Focus",
        bundle_3_badge: "",
        bundle_3_title: "Intensive Therapy Pack",
        bundle_3_desc: "For those who need intensive treatment. Multiple bottles of our signature Hair Therapy Oil for extended treatment periods or to share with loved ones.",
        bundle_3_item_1: "5x Hair Therapy Oil (250ml each)",
        bundle_3_item_2: "Free Scalp Massager",
        bundle_3_btn: "Shop Bundle",

        // Testimonials Section
        testimonials_label: "Testimonials",
        testimonials_title_1: "What Our Customers",
        testimonials_title_2: "Say",

        testimonial_1_text: "After just two weeks of using the Hair Therapy Oil, I noticed significantly less hair fall. The quality is unlike anything I've tried before. Truly organic!",
        testimonial_1_name: "Sarah M.",
        testimonial_1_title: "Verified Buyer",

        testimonial_2_text: "The Keratin Hair Mask transformed my damaged hair completely. It's so soft and manageable now. I've recommended GLANZ to all my friends!",
        testimonial_2_name: "Amina K.",
        testimonial_2_title: "Verified Buyer",

        testimonial_3_text: "The entire product line feels so luxurious. From the elegant packaging to the amazing scent, every detail shows quality. My hair has never looked better!",
        testimonial_3_name: "John D.",
        testimonial_3_title: "Verified Buyer",

        // CTA Section
        cta_title: "Ready to Transform Your Hair?",
        cta_text: "Join thousands of satisfied customers who have discovered the GLANZ difference. Start your journey to healthier, more beautiful hair today.",
        cta_btn_primary: "Shop Now",
        cta_btn_secondary: "Contact Us",

        // Contact Section
        contact_label: "Get in Touch",
        contact_title_1: "We'd Love to",
        contact_title_2: "Hear from You",
        contact_text: "Have questions about our products or need personalized recommendations? Our team is here to help you find the perfect hair care solution.",

        form_name_label: "Full Name",
        form_name_placeholder: "Enter your name",
        form_email_label: "Email Address",
        form_email_placeholder: "Enter your email",
        form_subject_label: "Subject",
        form_subject_placeholder: "How can we help?",
        form_message_label: "Message",
        form_message_placeholder: "Tell us more...",
        form_submit: "Send Message",

        // Footer
        footer_tagline: "Plant based hair care solutions for your wellness journey.",
        footer_quick_links: "Quick Links",
        footer_products: "Products",
        footer_support: "Support",
        footer_product_1: "Anti-Hair Loss Nourishing Shampoo",
        footer_product_2: "Keratin Hair Mask",
        footer_product_3: "Hair Therapy Oil",
        footer_product_4: "Silicone Scalp Brush",
        footer_support_1: "FAQ",
        footer_support_2: "Shipping Info",
        footer_support_3: "Returns",
        footer_support_4: "Privacy Policy",
        footer_copyright: "© 2026 GLANZ Wellness Hub. All rights reserved.",

        // WhatsApp
        whatsapp_tooltip: "Chat with us on WhatsApp",

        // Form Status Messages
        form_success: "Thank you! Your message has been sent successfully.",
        form_error: "Oops! Something went wrong. Please try again.",
        form_sending: "Sending...",


        // Results Section
        results_label: "Real Results",
        results_title_1: "See the",
        results_title_2: "Transformation",
        results_subtitle: "Drag the slider to witness the power of GLANZ.",
        results_before: "Before",
        results_after: "After",

        // FAQ Section
        faq_label: "Common Questions",
        faq_title_1: "We Have",
        faq_title_2: "Answers",

        faq_1_q: "Is this suitable for colored hair?",
        faq_1_a: "Yes! All GLANZ products are sulfate-free and color-safe. They are designed to protect and enhance your color while providing deep nourishment.",

        faq_2_q: "How often should I use the Hair Mask?",
        faq_2_a: "For best results, we recommend using the Keratin Repair Hair Mask 1-2 times per week. Leave it on for 15-20 minutes before rinsing thoroughly.",

        faq_3_q: "Are your products cruelty-free?",
        faq_3_a: "Absolutely! We are proud to be 100% cruelty-free. None of our products or ingredients are tested on animals.",

        faq_4_q: "How long until I see results?",
        faq_4_a: "Most customers notice softer, shinier hair after the first wash. For hair growth and structural repair, consistent use for 4-6 weeks yields the best transformation.",
    },

    sw: {
        // Navigation
        nav_home: "Nyumbani",
        nav_about: "Kuhusu",
        nav_products: "Bidhaa",
        nav_benefits: "Faida",
        nav_bundles: "Vifurushi",
        nav_contact: "Wasiliana",

        // Hero Section
        hero_badge: "Huduma ya Nywele Inayotokana na Mimea",
        hero_title_1: "Fungua",
        hero_title_2: "Mwangaza Wako wa Asili",
        hero_subtitle: "Furahia anasa ya huduma ya kitaalamu ya nywele na GLANZ Wellness Hub. Bidhaa zetu zilizoundwa kisayansi hulisha, kurekebisha, na kubadilisha nywele zako kutoka ndani.",
        hero_btn_primary: "Chunguza Mkusanyiko",
        hero_btn_secondary: "Gundua Falsafa Yetu",
        hero_scroll: "Sogeza chini kugundua",

        // About Section
        about_label: "Kuhusu GLANZ",
        about_tagline: "Ustawi na Uzuri Uliozama Katika Kila Unywele",
        about_text_1: "GLANZ Wellness Hub ilizaliwa kutokana na imani rahisi lakini yenye nguvu: kwamba huduma ya nywele inapaswa kuheshimu uzuri na ustawi. Kilichoanza kama shauku ya nywele zenye afya kilibadilika kuwa kujitolea kuunda bidhaa zinazojali sana ngozi ya kichwa, kuimarisha nywele, na kuinua taratibu za kila siku kuwa ibada zenye maana. Tunapatikana katika muungano wa ustawi, sayansi, na asili—tukiunda huduma ya nywele inayotokana na mimea, iliyoundwa kwa kufikiri ili kusaidia afya ya ngozi ya kichwa, kuhimiza ukuaji wa nywele zenye afya, na kurejesha usawa kutoka mzizi hadi ncha.",
        about_text_2: "Kama mwanzilishi, maono yangu yamekuwa ukuaji wa makusudi—kuendeleza huduma ya nywele ambayo ni ya asili, laini, yenye ufanisi, na iliyojikita katika utengenezaji wa kufikiri. Kila bidhaa inaonyesha kujitolea kwa ubora, usawa, na afya ya muda mrefu ya nywele, ikiongozwa na uelewa kwamba matokeo ya kweli yanatokana na uthabiti na utunzaji. Kwa sababu afya ya kweli ya nywele ni mazoezi, si ahadi tu.",
        about_text_3: "GLANZ ni zaidi ya chapa; ni falsafa ya uboreshaji, uadilifu, na kusudi. Tumejitolea kwa ubora, uwazi na ukuaji endelevu. Kutoka kwa uchaguzi wa viungo hadi maendeleo ya bidhaa, kila uamuzi unaongozwa na kujitolea kwetu kwa ubora, ustawi, na maisha marefu. Tunapoendelea kupanuka, lengo letu linabaki wazi: kujenga chapa ya ustawi wa nywele inayoaminika ambayo inatoa ubora bila maelewano na kuunda thamani ya kudumu kwa jamii yetu.",
        about_thank_you: "Asante kwa kuwa sehemu ya safari hii.",
        about_founder: "— Mwanzilishi, GLANZ Wellness Hub",

        // Mission & Vision
        mission_title: "Dhumuni Letu",
        mission_text: "Kuunda huduma ya nywele iliyoboreshwa, inayotokana na mimea ambayo inasaidia afya ya ngozi ya kichwa, inahimiza ukuaji wa nywele zenye afya, na kubadilisha taratibu za kila siku kuwa ibada za ustawi wa hali ya juu kupitia utengenezaji wa kufikiri na utunzaji wa makusudi.",
        vision_title: "Maono Yetu",
        vision_text: "Kuwa chapa ya ustawi wa nywele inayotambulika kimataifa inayojulikana kwa kuchanganya viungo vilivyohamasishwa na asili na kanuni za kisasa za utengenezaji—tukiweka kiwango kipya cha anasa, uadilifu, na afya ya muda mrefu ya nywele.",

        // Values
        values_title: "Maadili Yetu",
        value_1_title: "Ustawi Kwanza",
        value_1_text: "Tunapa kipaumbele afya ya ngozi ya kichwa, usawa, na matokeo ya muda mrefu katika kila utengenezaji na uamuzi.",
        value_2_title: "Uadilifu na Imani",
        value_2_text: "Tunaamini imani inajengwa kupitia uaminifu, ubora, na uthabiti—kutoka kwa viungo hadi uzoefu wa chapa.",
        value_3_title: "Uendelevu na Ubora",
        value_3_text: "Tumejitolea kwa mazoea ya kuwajibika na kujenga chapa iliyoundwa kwa maisha marefu na umuhimu wa kimataifa.",

        // Products Section
        products_label: "Mkusanyiko Wetu",
        products_title_1: "BIDHAA ASILIA ZA",
        products_title_2: "HUDUMA YA NYWELE",
        products_subtitle: "Gundua safu yetu iliyochaguliwa kwa uangalifu ya suluhisho za huduma ya nywele za kiwango cha kitaalamu",

        product_1_tag: "Inayouzwa Zaidi",
        product_1_title: "Anti-Hair Loss Nourishing Shampoo",
        product_1_desc: "Fomula ya kupambana na kuparuza iliyoundwa kukuza ukuaji wa nywele na kurekebisha tundu zilizoharibika. Imetajirisha na viungo vya kulisha kwa nywele zenye nguvu na afya.",
        product_1_size: "500ml",
        product_1_link: "Jifunze Zaidi →",

        product_2_tag: "Matibabu ya Kina",
        product_2_title: "Barakoa ya Nywele ya Ukarabati wa Keratini",
        product_2_desc: "Matibabu ya upya wa kina kwa ulaini wa hali ya juu. Inarejesha nywele zilizoharibika kwa kuingiza keratini ya kina kwa nywele laini na zinazoweza kudhibitiwa.",
        product_2_size: "500ml",
        product_2_link: "Jifunze Zaidi →",

        product_3_tag: "Mafuta Asilia",
        product_3_title: "Mafuta ya Tiba ya Nywele",
        product_3_desc: "Mafuta ya kifahari ya tiba yanayorejesha na kukuza ukuaji wa nywele. Fomula nyepesi inayofyonzwa haraka, ikiwacha nywele zilizolishwa bila kunona.",
        product_3_size: "250ml",
        product_3_link: "Jifunze Zaidi →",

        product_4_tag: "Kifaa",
        product_4_title: "Brush ya Kusugua Kichwa cha Silikoni",
        product_4_desc: "Kifaa cha kusugua kichwa cha ergonomic kilichoundwa kuchochea mzunguko wa damu na kuongeza ufyonzaji wa bidhaa. Kinapatikana kwa rangi nzuri za kawaida.",
        product_4_size: "Rangi 3",
        product_4_link: "Jifunze Zaidi →",

        // Benefits Section
        benefits_label: "Kwa Nini Uchague GLANZ",
        benefits_title_1: "Furahia",
        benefits_title_2: "Tofauti",

        benefit_1_title: "Ubora wa Asili",
        benefit_1_text: "Kila bidhaa imeundwa kwa viungo bora zaidi, kuhakikisha ubora wa kipekee na matokeo yanayoonekana.",

        benefit_2_title: "Fomula Laini",
        benefit_2_text: "Fomula zilizosawazishwa kwa uangalifu ambazo ni laini kwa nywele na kichwa chako huku zikitoa matokeo yenye nguvu.",

        benefit_3_title: "Imethibitishwa Kisayansi",
        benefit_3_text: "Inasaidiwa na utafiti na imeundwa kwa sayansi ya juu ya nywele ili kushughulikia wasiwasi maalum kwa ufanisi.",

        benefit_4_title: "Matokeo ya Kudumu",
        benefit_4_text: "Furahia matokeo ya mabadiliko yanayodumu, na matumizi ya mara kwa mara yanafunua nywele zenye afya zaidi na zenye nguvu.",

        // Gallery Section
        gallery_label: "Galeri",
        gallery_title_1: "Tazama Bidhaa Zetu",
        gallery_title_2: "Zikitumika",

        // Bundles Section
        bundles_label: "Suluhisho Kamili",
        bundles_title_1: "Vifurushi vya Huduma",
        bundles_title_2: "ya Nywele Vilivyochaguliwa",
        bundles_subtitle: "Pata uzoefu kamili wa GLANZ na vifurushi vyetu maalum vilivyochaguliwa",

        bundle_1_tag: "Seti ya Kuanza",
        bundle_1_badge: "",
        bundle_1_title: "Jozi Muhimu",
        bundle_1_desc: "Kamilifu kwa wanaoanza. Inajumuisha Shampoo yetu ya Kulisha Kupunguza Upotevu na Barakoa ya Nywele ya Ukarabati wa Keratini kwa utaratibu kamili wa kusafisha na matibabu.",
        bundle_1_item_1: "Shampoo ya Kulisha Kupunguza Upotevu (500ml)",
        bundle_1_item_2: "Barakoa ya Nywele ya Ukarabati wa Keratini (500ml)",
        bundle_1_btn: "Nunua Kifurushi",

        bundle_2_tag: "Huduma Kamili",
        bundle_2_badge: "Inayopendelewa Zaidi",
        bundle_2_title: "Tatu Bora",
        bundle_2_desc: "Kifurushi chetu kinachopendelewa zaidi. Kamilisha ibada yako ya huduma ya nywele na bidhaa zote tatu za msingi pamoja na vifaa viwili vya ziada vya kusugua kichwa kwa matokeo bora.",
        bundle_2_item_1: "Shampoo ya Kulisha Kupunguza Upotevu (500ml)",
        bundle_2_item_2: "Barakoa ya Nywele ya Ukarabati wa Keratini (500ml)",
        bundle_2_item_3: "Mafuta ya Tiba ya Nywele (250ml)",
        bundle_2_item_4: "Vifaa 2 vya Kusugua Kichwa vya Silikoni",
        bundle_2_btn: "Nunua Kifurushi",

        bundle_3_tag: "Lengo la Matibabu",
        bundle_3_badge: "",
        bundle_3_title: "Kifurushi cha Tiba ya Kina",
        bundle_3_desc: "Kwa wale wanaohitaji matibabu ya kina. Chupa nyingi za Mafuta yetu ya Tiba ya Nywele kwa vipindi vya matibabu vya muda mrefu au kushiriki na wapendwa.",
        bundle_3_item_1: "Mafuta 5 ya Tiba ya Nywele (250ml kila moja)",
        bundle_3_item_2: "Kifaa cha Kusugua Kichwa Bure",
        bundle_3_btn: "Nunua Kifurushi",

        // Testimonials Section
        testimonials_label: "Ushuhuda",
        testimonials_title_1: "Wateja Wetu",
        testimonials_title_2: "Wanasema Nini",

        testimonial_1_text: "Baada ya wiki mbili tu za kutumia Mafuta ya Tiba ya Nywele, niliangalia upungufu mkubwa wa kuanguka kwa nywele. Ubora ni tofauti na chochote nilichojaribu hapo awali. Kweli ni asilia!",
        testimonial_1_name: "Sarah M.",
        testimonial_1_title: "Mnunuzi Aliyethibitishwa",

        testimonial_2_text: "Barakoa ya Nywele ya Keratini ilibadilisha nywele zangu zilizoharibika kabisa. Ni laini sana na zinaweza kudhibitiwa sasa. Nimependekeza GLANZ kwa marafiki zangu wote!",
        testimonial_2_name: "Amina K.",
        testimonial_2_title: "Mnunuzi Aliyethibitishwa",

        testimonial_3_text: "Mstari wote wa bidhaa unahisi kifahari sana. Kutoka kwa ufungaji wa kifahari hadi harufu ya ajabu, kila undani unaonyesha ubora. Nywele zangu hazijawahi kuonekana vizuri zaidi!",
        testimonial_3_name: "John D.",
        testimonial_3_title: "Mnunuzi Aliyethibitishwa",

        // CTA Section
        cta_title: "Uko Tayari Kubadilisha Nywele Zako?",
        cta_text: "Jiunge na maelfu ya wateja walioridhi ambao wamegundua tofauti ya GLANZ. Anza safari yako kwa nywele zenye afya zaidi na nzuri zaidi leo.",
        cta_btn_primary: "Nunua Sasa",
        cta_btn_secondary: "Wasiliana Nasi",

        // Contact Section
        contact_label: "Wasiliana",
        contact_title_1: "Tungependa",
        contact_title_2: "Kusikia Kutoka Kwako",
        contact_text: "Una maswali kuhusu bidhaa zetu au unahitaji mapendekezo ya kibinafsi? Timu yetu iko hapa kukusaidia kupata suluhisho kamilifu la huduma ya nywele.",

        form_name_label: "Jina Kamili",
        form_name_placeholder: "Ingiza jina lako",
        form_email_label: "Anwani ya Barua Pepe",
        form_email_placeholder: "Ingiza barua pepe yako",
        form_subject_label: "Mada",
        form_subject_placeholder: "Tunaweza kukusaidia vipi?",
        form_message_label: "Ujumbe",
        form_message_placeholder: "Tuambie zaidi...",
        form_submit: "Tuma Ujumbe",

        // Footer
        footer_tagline: "Suluhisho za huduma ya nywele zinazotokana na mimea kwa safari yako ya ustawi.",
        footer_quick_links: "Viungo vya Haraka",
        footer_products: "Bidhaa",
        footer_support: "Msaada",
        footer_product_1: "Anti-Hair Loss Nourishing Shampoo",
        footer_product_2: "Barakoa ya Keratini",
        footer_product_3: "Mafuta ya Tiba ya Nywele",
        footer_product_4: "Brush ya Kusugua Kichwa",
        footer_support_1: "Maswali Yanayoulizwa Mara kwa Mara",
        footer_support_2: "Taarifa za Usafirishaji",
        footer_support_3: "Kurudisha",
        footer_support_4: "Sera ya Faragha",
        footer_copyright: "© 2026 GLANZ Wellness Hub. Haki zote zimehifadhiwa.",

        // WhatsApp
        whatsapp_tooltip: "Piga gumzo nasi kwenye WhatsApp",

        // Form Status Messages
        form_success: "Asante! Ujumbe wako umetumwa kikamilifu.",
        form_error: "Samahani! Kuna tatizo. Tafadhali jaribu tena.",
        form_sending: "Inatuma...",


        // Results Section
        results_label: "Matokeo Halisi",
        results_title_1: "Ona",
        results_title_2: "Mabadiliko",
        results_subtitle: "Vuta kitelezi ili kushuhudia nguvu ya GLANZ.",
        results_before: "Kabla",
        results_after: "Baada",

        // FAQ Section
        faq_label: "Maswali ya Kawaida",
        faq_title_1: "Tuna",
        faq_title_2: "Majibu",

        faq_1_q: "Je, hii inafaa kwa nywele zilizotiwa rangi?",
        faq_1_a: "Ndiyo! Bidhaa zote za GLANZ hazina sulfate na ni salama kwa rangi. Zimeundwa kulinda na kuboresha rangi yako huku zikitoa lishe ya kina.",

        faq_2_q: "Je, nitumie mara ngapi Barakoa ya Nywele?",
        faq_2_a: "Kwa matokeo bora, tunapendekeza kutumia Barakoa ya Nywele ya Ukarabati wa Keratini mara 1-2 kwa wiki. Iache kwa dakika 15-20 kabla ya kuosha kabisa.",

        faq_3_q: "Je, bidhaa zenu haziwadhuru wanyama?",
        faq_3_a: "Kabisa! Tunajivunia kuwa 100% hatudhuru wanyama. Hakuna bidhaa zetu au viungo vinavyopimwa kwa wanyama.",

        faq_4_q: "Itachukua muda gani kuona matokeo?",
        faq_4_a: "Wateja wengi hugundua nywele laini na zenye kung'aa baada ya kuosha kwa kwanza. Kwa ukuaji wa nywele na ukarabati wa muundo, matumizi thabiti kwa wiki 4-6 hutoa mabadiliko bora.",
    }
};
